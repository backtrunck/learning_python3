...
def obter_licitacoes_um_pra_muitos(conexao,alvo_principal,alvos_secundarios):
    ...
    for licitacao in licitacoes_alvo:
...
def obter_licitacoes_muitos_pra_muitos(conexao,alvos_principais):
    ...
    if licitacoes_alvos_principais:
        ...
        for licitacao in licitacoes_alvos_principais:
...
def obter_licitacoes(cnpjs,conexao):
    ...
    """
    if len(cnpjs) == 1:
        consulta = "SELECT DISTINCT unidade, datahomologacao, processo, modalidade " + \
                    "FROM TCM_BA.tcmba_licitacoes " + \
                    "WHERE `CNPJ-CPF`= %s"
    else:
        #todas as licitações com participação do cnpjs secundários
        consulta = "SELECT DISTINCT unidade, datahomologacao, processo, modalidade " + \
            "FROM TCM_BA.tcmba_licitacoes " + \
            gerar_filtro(cnpjs)
...
<listas_cnpjs>")
    ...
    try:
        ...
        if erro.errno == mysql.connector.errorcode.ER_ACCESS_DENIED_ERROR:
            ...
            print("Você não tem permissão de acessar o banco {}".format(BANCO_DADOS))
            exit(erro.errno)
        else:
            print("Erro: {}".format(erro))
            exit(erro.errno)
    except mysql.connector.InterfaceError as erro:
            print("Não foi possivel conectar com o servidor: {}".format(erro.msg))
            exit(erro.errno)
    ...
    if len(cnpjs_principais) == 1:
        ...
        #todos com todos
        licitacoes_comuns = obter_licitacoes_muitos_pra_muitos(conexao,cnpjs_principais)
    ...
    if licitacoes_comuns:
        ...
        escritor = csv.writer(arquivo_csv,delimiter=";", quotechar='"',quoting = csv.QUOTE_ALL)
